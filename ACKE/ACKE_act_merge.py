...
class ACKE(torch.nn.Module):
    ...
    def cal_activation_loss_new(self, new_weight_output, original_weight_output, deact_mask, tokens, config=None):
        ...
        #return dist_loss, dist_similarity
...
class ACKEAdapter(torch.nn.Module):
    def __init__(self, config, layer, transpose):
        ...
        #测试是否可以让D4S更新正常，是的，成功了，设定新层确保更新
        #self.register_parameter('new_weight', nn.Parameter(self.weight.clone(), requires_grad=True))
        #self.register_parameter('new_weight', nn.Parameter(self.new_weight.clone(), requires_grad=True))
        self.register_parameter('new_weight', nn.Parameter(self.new_weight, requires_grad=True))#这个才是，不能是clone
    ...
    def forward(self, *args):
        if self.editing:
            ...
            new_weight_layer_output = self.layer(*args)
