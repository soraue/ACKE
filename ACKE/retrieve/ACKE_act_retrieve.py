...
def euc_diff_target_similarity(output_diff, target_vector, act_mask=None, infer=False):
    
    direct_similarity = COSSIM(output_diff, target_vector, dim=-1)
    
    similarity = direct_similarity
    
    if infer and similarity.size(1) > 100:
        topk = torch.topk(similarity, k=1, largest=True)
        return topk.values.mean()
...
class ACKE(torch.nn.Module):
    def __init__(self, config, model, device):
        ...
        if type(adapter_layer) is not ACKEAdapter:
            setattr(self.edit_module, self.layer_name, ACKEAdapter(config, adapter_layer, transpose=transpose))
    ...
    def get_adapter_layer(self):
        adapter_layer = getattr(self.edit_module, self.layer_name)
        assert type(adapter_layer) is ACKEAdapter, print('Adapter Layer is not added correctly....')
    ...
    def edit(self, config, tokens, act_mask=None, deact_mask=None):
        ...
        for i in range(config.n_iter):
            ...
            if i == 0:
                ...
                #optimizer = torch.optim.Adam([self.get_adapter_layer().new_weight], config.edit_lr, weight_decay=1e-5)
            ...
            act_loss_ne = self.cal_activation_loss_new(self.get_adapter_layer().new_weight_layer_output,  self.get_adapter_layer().original_layer_output, deact_mask = deact_mask, tokens = tokens, config = config)

            '''if act_loss < 5:
                act_loss_ne = self.cal_activation_loss_new(self.get_adapter_layer().new_weight_layer_output,  self.get_adapter_layer().original_layer_output, deact_mask = deact_mask, tokens = tokens, config = config) #* 0.01
                act_loss += act_loss_ne'''
                #act_loss -= K
            act_loss = act_loss * 0 + act_loss_ne * 10
            loss = ft_loss * 0 + act_loss.to(ft_loss.device)
            #loss = ft_loss * 0 + act_loss.to(ft_loss.device) + act_loss_ne.to(ft_loss.device)
            #loss = ft_loss * 0 + act_loss.to(ft_loss.device)
            ...
            if self.config.retrieve and self.get_adapter_layer().merge_cnt > 0 and self.config.replay:
                ...
                #print(
                #    f"loss {np.round(loss.item(), 3)} = {np.round(act_loss.item(), 3)}"
                #)
            else:
                print(
    ...
    def cal_activation_loss_new(self, new_weight_output, original_weight_output, deact_mask, tokens, config=None):
        assert isinstance(new_weight_output, torch.Tensor), "new_weight_output must be a torch.Tensor"
        assert isinstance(original_weight_output, torch.Tensor), "original_weight_output must be a torch.Tensor"
        assert isinstance(tokens['labels'], torch.Tensor), "tokens['labels'] must be a torch.Tensor"
        i = 0
        k = 1
        len_temp = original_weight_output.shape[0] / k - 1
        target_ids = tokens['labels'].clone()
        #no_target_ids = target_ids[int(i-k):, ...]
        #target_ids = target_ids[int(i*len_temp):int((i+1)*len_temp), ...]
                    
        pad_token_id = 0
        target_ids[target_ids == -100] = pad_token_id
        embeddings = self.model.get_input_embeddings()
        target_vectors = embeddings(target_ids)  # [batch_size, sequence_length, hidden_size]
        target_vectors = target_vectors[int(i*len_temp):int((i+1)*len_temp), ...]
        target_similarity = euc_diff_target_similarity(
            new_weight_output[int(i*len_temp):int((i+1)*len_temp), ...] - original_weight_output[int(i*len_temp):int((i+1)*len_temp), ...],
            target_vectors - original_weight_output[int(i*len_temp):int((i+1)*len_temp), ...]
        )
        #print(target_similarity)
        
        no_target_vectors = target_vectors[int(i-k):, ...]
        no_target_similarity = euc_diff_target_similarity(
            new_weight_output[int(i-k):, ...] - original_weight_output[int(i-k):, ...],
            no_target_vectors - original_weight_output[int(i-k):, ...]
        )
        #print(no_target_similarity)
        
        dist_similarity = euc_diff_target_similarity(
            new_weight_output[int(i*len_temp):int((i+1)*len_temp), ...],
            original_weight_output[int(i*len_temp):int((i+1)*len_temp), ...]
        )
        
        no_dist_similarity = euc_diff_target_similarity(
            new_weight_output[int(i-k):, ...],
            original_weight_output[int(i-k):, ...]
        )

        dist_loss = torch.relu(dist_similarity + 0.5)# if (dist_similarity - 0.2) > 0 else 0

        no_dist_loss = torch.relu(0.8 - no_dist_similarity)# if (0.8 - no_dist_similarity) > 0 else 0
        
        print("dist_loss:", dist_loss.item())
        print("no_dist_loss:", no_dist_loss.item())

        return dist_loss + no_dist_loss
...
class ACKEAdapter(torch.nn.Module):
    def __init__(self, config, layer, transpose):
        ...
        self.register_parameter('new_weight', nn.Parameter(self.new_weight, requires_grad=True))
    ...
    def forward(self, *args):
        if self.editing:
            ...
            if not self.config.retrieve:
                ...
                i = 0
                k = 1
                len_temp = original_layer_output.shape[0] - 1
                cos_dist1 = euc_diff_target_similarity(
                ...
                if cosactivate == False:
                    if min_dist.item() < threshold:
                        layer_out = original_layer_output
                        #globalact.actglo = False
                    else:
                        layer_out = original_layer_output
                        #globalact.actglo = True

                memory_retrieve_weight = self.memory_weight[0]
                memory_weight_layer_output = F.linear(*args, memory_retrieve_weight)
                currennt_cosi = 0
                current_cosact = euc_diff_target_similarity(memory_weight_layer_output, original_layer_output)
                fallcount = 0
                for i in range(len(self.memory_weight)):
                    ...
                    dist_s = euc_diff_target_similarity(memory_weight_layer_output, original_layer_output)
                    if dist.dim() > 0:
                        dist = dist.mean()
                    if dist_s < 0.45:
                        globalact.actglo = True
                        if i == 0:
                            current_cosact = dist_s
                            currennt_cosi = i
                            globalact.actnum = i
                        elif current_cosact > dist_s:
                            current_cosact = dist_s
                            currennt_cosi = i
                            globalact.actnum = i
                    else:
                ...
                if fallcount == 2:
                    globalact.actglo = False
                with open ('./nnocheck.txt','a') as file:
                    file.write(str(globalact.actnum))
                    file.write(str(currennt_cosi))
                    file.write('\n')
                    file.close()
