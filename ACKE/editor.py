...
class BaseEditor:
    ...
    def batch_edit(self,
        ...
        for record_chunks in _chunks(requests, self.hparams.batch_size):
            ...
            if sequential_edit:
                ...
                if self.alg_name == 'KN' or self.alg_name == 'GRACE' or self.alg_name == 'WISE' or self.alg_name == 'ACKE':
                    with torch.no_grad():
                        weights_copy()
    ...
    def edit_requests(self,
        ...
        if sequential_edit:
            ...
            if self.alg_name == 'ACKE' and hasattr(self.hparams, 'save_path') and self.hparams.save_path:
                print("Start saving the ACKE model!")
                edited_model.save(self.hparams.save_path)
            for i, request in enumerate(requests):
                with open ('./nnocheck.txt','a') as file:
                    file.write(str(i))
                    file.write('\n')
                with open ('./getact.txt','a') as file:
                    file.write(str(i))
                    file.write('\n')
